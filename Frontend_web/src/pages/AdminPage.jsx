...
const AdminPage = () => {
  ...
  const denyClaim = async (requestId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_CONFIG.BASE_URL}/claims/deny/${requestId}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.ok) {
  ...
  const handleReReview = async (itemId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_CONFIG.BASE_URL}/claims/rereview/${itemId}`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUpdateItem = async (updatedItem) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_CONFIG.BASE_URL}/items/update/${updatedItem.itemID}`, {
        method: "PUT",
        headers: {
  ...
  const markAsUnclaimed = async (itemId) => {
    ...
    const token = localStorage.getItem("token");
    const response = await fetch(`${API_CONFIG.BASE_URL}/items/${itemId}/unclaim`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const handleUpdateUser = async (updatedUser) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_CONFIG.BASE_URL}/user/update/${updatedUser.schoolId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteUser = async (schoolId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_CONFIG.BASE_URL}/user/delete/${schoolId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.ok) {
